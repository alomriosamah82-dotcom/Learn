...
// ===== حل مشكلة ظهور الإحصائيات عند التحميل الأول =====
document.addEventListener('DOMContentLoaded', function() {
    // مراقبة تغييرات القسم وتحديث الإحصائيات عند العودة للوحة التحكم
    const originalSwitchSection = window.switchSection;
    
    window.switchSection = function(section) {
        originalSwitchSection(section);
        
        // إذا كان القسم هو لوحة التحكم، قم بتحديث الإحصائيات فوراً
        if (section === 'dashboard') {
            setTimeout(() => {
                updateDashboardStats();
            }, 100);
...
// تأكد من تحديث الإحصائيات بعد تحميل البيانات مباشرة
setTimeout(() => {
...
// تحسين دالة updateDashboardStats لضمان التحديث الكامل
function updateDashboardStats() {
    try {
        // تحديث إحصائيات لوحة التحكم
        const stats = {
        ...
        // استخدام querySelector الآمن
        const learningElement = document.getElementById('learning-count');
        const youtubeElement = document.getElementById('youtube-count');
        const workElement = document.getElementById('work-count');
        const toolsElement = document.getElementById('tools-count');
        
        if (learningElement) learningElement.textContent = stats.learning;
        if (youtubeElement) youtubeElement.textContent = stats.youtube;
        if (workElement) workElement.textContent = stats.work;
        if (toolsElement) toolsElement.textContent = stats.tools;
        
        // إضافة تأثير ظهور تدريجي للإحصائيات
        const statElements = [learningElement, youtubeElement, workElement, toolsElement];
...
// تحديث إضافي عند اكتمال تحميل الصفحة
...
// التأكد من تحديث الإحصائيات عند تغيير حجم النافذة (لحل مشاكل التصيير)
...
// تحديث الإحصائيات عند ظهور قسم لوحة التحكم
const observer = new MutationObserver(function(mutations) {
    mutations.forEach(function(mutation) {
        if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
            const target = mutation.target;
            if (target.id === 'dashboard-section' && target.classList.contains('active')) {
                setTimeout(() => {
                    updateDashboardStats();
                }, 50);
            }
...
// بدء المراقبة بعد تهيئة التطبيق
setTimeout(() => {
...
// ===== نظام زر الرجوع إلى الأعلى (إضافة آمنة) =====
(function() {
    'use strict';
    
    console.log('🚀 بدء تحميل نظام الرجوع إلى الأعلى...');
    
    // الانتظار حتى اكتمال تحميل DOM
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initializeBackToTopSystem);
    } else {
        setTimeout(initializeBackToTopSystem, 100);
    }
    
    function initializeBackToTopSystem() {
        try {
            // الانتظار حتى يكتمل تحميل جميع المكونات
            setTimeout(() => {
                createBackToTopButton();
                setupBackToTopEvents();
                integrateWithExistingSystem();
                
                console.log('✅ تم تحميل نظام الرجوع إلى الأعلى بنجاح');
            }, 500);
        } catch (error) {
            console.error('❌ خطأ في تحميل نظام الرجوع إلى الأعلى:', error);
        }
    }
    ...
    function createBackToTopButton() {
        try {
            ...
            if (document.getElementById('back-to-top')) {
                console.log('ℹ️ زر الرجوع إلى الأعلى موجود مسبقاً');
                return;
            }
            ...
            console.log('✅ تم إنشاء زر الرجوع إلى الأعلى');
            
        } catch (error) {
            console.error('❌ خطأ في إنشاء زر الرجوع إلى الأعلى:', error);
        }
    }
        ...
        try {
            ...
            function toggleBackToTopButton() {
                try {
                    ...
                    console.error('❌ خطأ في تبديل زر الرجوع للأعلى:', error);
                }
            }
            
            // وظيفة الرجوع السلس للأعلى
            function scrollToTop() {
                try {
                    ...
                    console.error('❌ خطأ في الرجوع إلى الأعلى:', error);
                    window.scrollTo(0, 0);
                }
            }
            ...
            console.log('✅ تم إعداد أحداث زر الرجوع إلى الأعلى');
            
        } catch (error) {
            console.error('❌ خطأ في إعداد أحداث زر الرجوع إلى الأعلى:', error);
        }
    }
    
    function integrateWithExistingSystem() {
        try {
            // الدمج مع نظام تبديل الأقسام الموجود
            if (typeof window.switchSection === 'function') {
                const originalSwitchSection = window.switchSection;
                window.switchSection = function(section) {
                    const result = originalSwitchSection.apply(this, arguments);
                    
                    // تحديث زر الرجوع للأعلى بعد تبديل القسم
                    setTimeout(() => {
                        const backToTopBtn = document.getElementById('back-to-top');
                        if (backToTopBtn) {
                            if (window.pageYOffset > 300) {
                ...
                console.log('✅ تم الدمج مع نظام تبديل الأقسام');
            }
            
            // الدمج مع نظام الوضع الداكن/الفاتح
            if (typeof window.toggleTheme === 'function') {
                const originalToggleTheme = window.toggleTheme;
                window.toggleTheme = function() {
                    const result = originalToggleTheme.apply(this, arguments);
                    
                    // تحديث ألوان الزر بعد تغيير الوضع
                    setTimeout(updateButtonColors, 100);
                    
                    return result;
                };
                
                console.log('✅ تم الدمج مع نظام تبديل الوضع');
            }
            ...
            function updateButtonColors() {
                ...
                if (backToTopBtn) {
                    ...
                    backToTopBtn.style.background = isDark ? 
            ...
            console.error('❌ خطأ في الدمج مع النظام الحالي:', error);
        }
    }
    
    // التعامل مع التغييرات الديناميكية في الصفحة
    const observePageChanges = function() {
        try {
            const observer = new MutationObserver(function(mutations) {
                mutations.forEach(function(mutation) {
                    if (mutation.type === 'childList') {
                        // التحقق من وجود الزر بعد أي تغيير في الـ DOM
                        setTimeout(() => {
                            if (!document.getElementById('back-to-top')) {
                                createBackToTopButton();
                                setupBackToTopEvents();
    ...
    // بدء المراقبة بعد التأكد من اكتمال التحميل
    setTimeout(observePageChanges, 1000);
    
})();

// ===== التأكد من السلامة والاستقرار =====
(function() {
    'use strict';
    ...
    setTimeout(() => {
        const checkSystemStability = function() {
            try {
                // التحقق من العناصر المكررة
                const duplicateButtons = document.querySelectorAll('#back-to-top');
                if (duplicateButtons.length > 1) {
                    console.warn('⚠️ اكتشاف ازدواجية في زر الرجوع للأعلى، جاري الإصلاح...');
                    for (let i = 1; i < duplicateButtons.length; i++) {
                        duplicateButtons[i].remove();
                    }
                }
                
                // التحقق من عدم التداخل مع شريط البار السفلي
                const bottomBar = document.querySelector('.bottom-action-bar');
                const backToTopBtn = document.getElementById('back-to-top');
                
                if (bottomBar && backToTopBtn) {
                    const bottomBarRect = bottomBar.getBoundingClientRect();
                    const buttonRect = backToTopBtn.getBoundingClientRect();
                    
                    if (buttonRect.bottom > bottomBarRect.top - 10) {
                        backToTopBtn.style.bottom = (bottomBarRect.height + 20) + 'px';
                    }
                }
                
                console.log('✅ النظام مستقر وجاهز للعمل');
                
            } catch (error) {
                console.error('❌ خطأ في التحقق من الاستقرار:', error);
            }
        };
        
        checkSystemStability();
        
        // التحقق الدوري كل 5 ثوانٍ
        setInterval(checkSystemStability, 5000);
